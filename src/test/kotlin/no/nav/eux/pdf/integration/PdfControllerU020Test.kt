...
class PdfControllerU020Test : AbstractPdfApiImplTest() {
    ...
    inner class SuccessfulPdfGeneration {
        ...
        fun shouldGenerateCompleteU020Pdf() {
            ...
            logTestInfo("Testing complete U020 PDF generation", endpoint)
    ...
    inner class ResponseValidation {
        ...
        fun shouldReturnCorrectHttpHeaders() {
            ...
            val response = callPdfEndpoint(endpoint)
            assertSuccessfulPdfResponse(response)
            validateContentDispositionHeader(response, EXPECTED_FILENAME_U020)
    ...
    private fun verifyAllSubdocumentsProcessed() {
        verifySubdocumentsProcessed(
            expectedCount = 3,
            subdocumentFilter = { it.contains("/Subdocuments/subdoc_") },
            documentType = "U020"
        )
    }
}